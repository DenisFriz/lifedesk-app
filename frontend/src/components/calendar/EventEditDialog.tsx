...
export default function EventEditDialog({ event, open, onOpenChange }) {
  ...
  const handleUpdateEvent = async ({ id, data }: { id: string; data: any }) => {
    try {
  ...
  const handleSave = () => {
    handleUpdateEvent({ id: event.id, data: formData })
  }
  ...
  const handleDeleteConfirm = deleteType => {
    if (deleteType === 'single') {
      ...
      const excludedDates = Array.isArray(event.excluded_dates) ? [...event.excluded_dates] : []
      const dateToExclude = event.start_date
      ...
      if (!excludedDates.includes(dateToExclude)) {
        ...
        backend.entities.Event.update(event.id, { excluded_dates: excludedDates }).then(() => {
