...
export default function Hobbies() {
  ...
  const { data: hobbies = [] } = useQuery({
    queryKey: ['hobbies'],
    queryFn: () => backend.entities.Hobby.filter({ is_deleted: false }, '-created_date')
  })

  const atLimit = hobbyLimit !== Infinity && hobbies.length >= hobbyLimit
  const isOverLimit = idx => hobbyLimit !== Infinity && idx >= hobbyLimit

  const createMutation = useMutation({
    mutationFn: data => backend.entities.Hobby.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => backend.entities.Hobby.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['hobbies'] })
      setShowForm(false)
  ...
  const deleteMutation = useMutation({
    mutationFn: id => backend.entities.Hobby.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['hobbies'] })
    }
  })
