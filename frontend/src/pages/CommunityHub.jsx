...
export default function CommunityHub() {
  ...
  const { data: ideas = [] } = useQuery({
    ...
    queryFn: () => backend.entities.CommunityIdea.list()
  })

  // Fetch user's votes
  const { data: myVotes = [] } = useQuery({
    queryKey: ['community_votes', user?.email],
    queryFn: () => backend.entities.CommunityVote.filter({ user_email: user?.email }),
    enabled: !!user?.email
  })
  ...
  const createIdea = useMutation({
    mutationFn: data => backend.entities.CommunityIdea.create(data),
    onSuccess: () => {
  ...
  const deleteIdea = useMutation({
    mutationFn: id => backend.entities.CommunityIdea.delete(id),
  ...
  useEffect(() => {
    if (myNotifications.length > 0) {
      const seen = new Set()
      const deduped = myNotifications.filter(n => {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === 'likes') list.sort((a, b) => (b.likes_count || 0) - (a.likes_count || 0))
