...
export default function Workouts() {
  ...
  const createMutation = useMutation({
    mutationFn: data => backend.entities.Workout.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => backend.entities.Workout.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workouts'] })
      setShowForm(false)
      setEditingWorkout(null)
  ...
  const deleteMutation = useMutation({
    mutationFn: id => backend.entities.Workout.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workouts'] })
    }
  })
          ...
          {filteredWorkouts.length === 0 ? (
            ...
            filteredWorkouts.map(workout => {
              ...
              return overLimit ? (
                ...
                <React.Fragment key={workout.id}>{card}</React.Fragment>
              )
            })
          )}
        </div>
...
function WorkoutForm({ open, onClose, onSubmit, workout, isLoading }) {
  ...
  const handleSubmit = e => {
    ...
    data.calories_burned = data.calories_burned ? parseFloat(data.calories_burned) : null

    // Clean up exercises - remove empty ones and parse numbers
