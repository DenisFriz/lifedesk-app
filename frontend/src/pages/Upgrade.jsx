...
export default function Upgrade() {
  ...
  const { data: subscription = null, error: subError } = useQuery({
    queryKey: ['subscription', user?.email],
    queryFn: async () => {
      if (!user?.email) return null
      try {
        const subs = await backend.entities.Subscription.list()
  ...
  const handleBillingPortal = async () => {
    ...
    const res = await backend.functions.invoke('createBillingPortalSession', {
  ...
  const handleDowngradeToPlus = async () => {
    ...
    const res = await backend.functions.invoke('downgradeToPlan', {
      plan_name: 'plus',
      return_url: `${window.location.origin}/Profile`
    })
    setLoading(null)
  ...
  const handleUpgrade = async () => {
    setLoading('plus')
    const res = await backend.functions.invoke('createCheckoutSession', {
      plan_name: 'plus',
