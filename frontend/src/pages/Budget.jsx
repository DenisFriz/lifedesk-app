...
export default function Budget() {
  ...
  const updateRecurringIncomeMutation = useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      const previousIncome = queryClient.getQueryData(['recurring-income'])
      queryClient.setQueryData(['recurring-income'], old =>
  ...
  const updateRecurringExpenseMutation = useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      const previousExpenses = queryClient.getQueryData(['recurring-expenses'])
      queryClient.setQueryData(['recurring-expenses'], old =>
        old?.map(item => (item.id === id ? { ...item, ...data } : item))
      )
  ...
  const saveEdit = (item, field, isIncome) => {
    ...
    if (field === 'business_id') {
      ...
      // Also clear the category when changing life area
      const updateData = { [field]: value, category: '' }
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#f4f7fb' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg border border-slate-200 mb-6">
          ...
          >
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-semibold text-slate-900">Recurring Income & Expenses</h2>
            </div>
                            ...
                            >
                              ...
                              <td className="px-4 py-3 align-top w-32">
                                {editingField === `${expense.id}-amount` ? (
                                  ...
                                  <div
                                    onClick={() => startEdit(expense.id, 'amount', expense.amount)}
                              ...
                              <td className="px-4 py-3 align-top w-16">
                                <Button
                                  ...
                                  size="icon"
                                  className="h-8 w-8"
                                  onClick={() => deleteRecurringExpenseMutation.mutate(expense.id)}
                                >
                                  <Trash2 className="h-4 w-4 text-rose-500" />
                                </Button>
                              </td>
