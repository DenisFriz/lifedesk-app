...
export default function BusinessGoals() {
  ...
  const { data: business } = useQuery({
    ...
    queryFn: () => backend.entities.Business.filter({ id: businessId }).then(res => res[0]),
    enabled: !!businessId
  })
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {isScrolled && (
          <div className="lg:hidden sticky top-[52px] z-20 bg-white border-b border-slate-200 shadow-sm -mx-4 sm:-mx-6 px-4 sm:px-6">
            <div className="py-3">
              <h1 className="businessgoals-sticky-title text-sm font-normal text-slate-900 text-center">
                {business?.name} - Goals
              </h1>
            </div>
          </div>
        ...
        <div ref={headerRef} className="py-6 sm:py-8">
          <h1 className="businessgoals-page-title text-3xl sm:text-4xl font-bold text-slate-900 text-center lg:text-left mb-2 flex items-center justify-center lg:justify-start gap-3">
