...
export default function TimeReports() {
  ...
  const { data: entries = [] } = useQuery<TimeEntry[]>({
    ...
    queryFn: () => backend.entities.TimeEntry.list('-date') as Promise<TimeEntry[]>
  })

  const { data: clients = [] } = useQuery<Client[]>({
    queryKey: ['clients'],
    queryFn: () => backend.entities.Client.list('name') as Promise<Client[]>
  })

  const { data: projects = [] } = useQuery<Project[]>({
    queryKey: ['projects'],
    queryFn: () => backend.entities.Project.list('name') as Promise<Project[]>
  })
