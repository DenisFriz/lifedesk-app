...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        await apiFetch<{ message: string }>('POST', '/email/verify-email', { token })
        if (cancelled) return
        setStatus('success')
  ...
  return (
    <>
      <Helmet>
        <title>Verify Email | LifeDesk</title>
          ...
          <div className="p-6 pt-12 pb-10 px-12 space-y-6 text-center">
            ...
            {status === 'success' && (
              <p className="text-sm text-slate-500">
                Redirecting you {isAuthenticated ? 'to the app' : 'to login'}...
              </p>
            )}
