...
function OfflineAccountForm({ account, onSubmit, onClose }) {
...
export default function Accounts() {
  ...
  const { data: offlineAccounts = [] } = useQuery({
    ...
    queryFn: () => backend.entities.OfflineAccount.filter({ is_deleted: false }, '-created_date')
  })
  ...
  const { data: bankAccounts = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await backend.functions.invoke('plaid', { action: 'get_balances' })
  ...
  const createMutation = useMutation({
    mutationFn: data => backend.entities.OfflineAccount.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => backend.entities.OfflineAccount.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['offlineAccounts'] })
  ...
  const deleteMutation = useMutation({
    mutationFn: id => backend.entities.OfflineAccount.delete(id),
        ...
        {offlineAccounts.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              <p className="text-2xl font-bold text-blue-600">{formatCurrency(totalBankBalance)}</p>
          ...
          {offlineAccounts.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200 text-center py-10 text-slate-400">
              ...
              <p className="text-xs mt-1">
                Add accounts like savings, cash, or accounts that can't be connected automatically.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
