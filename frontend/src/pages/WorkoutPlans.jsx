...
import { Plus, Trash2, Pencil, Play, Calendar as CalendarIcon, Dumbbell, Lock } from 'lucide-react'
...
export default function WorkoutPlans() {
  ...
  const { data: plans = [] } = useQuery({
    ...
    queryFn: () => backend.entities.WorkoutPlan.filter({ is_deleted: false }, '-created_date')
  })

  const atLimit = planLimit !== Infinity && plans.length >= planLimit

  const createMutation = useMutation({
    mutationFn: data => backend.entities.WorkoutPlan.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => backend.entities.WorkoutPlan.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workoutPlans'] })
      setShowForm(false)
      setEditingPlan(null)
  ...
  const deleteMutation = useMutation({
    mutationFn: id => backend.entities.WorkoutPlan.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workoutPlans'] })
    }
  })
