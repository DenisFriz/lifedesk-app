...
export async function aiOllamaChat(req: Request, res: Response) {
  try {
    const user = req.user;
...
User Question: ${question}`;
    ...
    if (file_urls?.length) {
      ...
      const images = await Promise.all(
        file_urls.map((url: string) => urlToBase64(url)),
      );
    ...
    if (!response.ok) {
      return res.status(500).json({
        error: `Ollama API error: ${response.statusText} - ${rawBody}`,
      });
    }
    ...
    try {
      ...
      return res.status(500).json({
        error: 'Ollama API returned an invalid response',
      });
    }
